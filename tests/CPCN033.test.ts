...
test('CPCN033', async ({ page }) => {
    ...
    await page.getByLabel('Regular price (S/)').fill('0.01');
    await page.getByLabel('Sale price (S/)').click();
    await page.getByLabel('Sale price (S/)').fill('0');
    ...
    await page.getByPlaceholder('Length').fill('0');
    await page.getByPlaceholder('Width').click();
    await page.getByPlaceholder('Width').fill('0');
    await page.getByPlaceholder('Height').click();
    await page.getByPlaceholder('Height').fill('0');
    await page.getByRole('button', { name: 'Publish', exact: true }).click();
    await expect(page.getByLabel('Product name')).toHaveValue('a');
    await expect(page.getByLabel('Regular price (S/)')).toHaveValue('0.01');
    await expect(page.getByLabel('Sale price (S/)')).toHaveValue('0');
    await page.getByRole('link', { name: 'Shipping' }).click();
    await expect(page.getByPlaceholder('0')).toHaveValue('0');
    await expect(page.getByPlaceholder('Length')).toHaveValue('0');
    await expect(page.getByPlaceholder('Width')).toHaveValue('0');
    await expect(page.getByPlaceholder('Height')).toHaveValue('0');
